...
const cellProps = <T extends Record<string, unknown>>(props: any, {cell}: Meta<T, { cell: Cell<T> }>) =>
  getStyles(props, cell.column && cell.column.align)
...
export interface TableProps<T extends Record<string, unknown>>
  extends PropsWithChildren<TableProperties<T>>,
    UseUpdateTableOptions<T> {
}

export function Table<T extends Record<string, unknown>>(props: TableProps<T>): ReactElement {
  const {name, columns, onAdd, onDelete, onEdit, onClick, updateData} = props
  ...
  useEffect(() => {
    const {sortBy, filters, pageSize, hiddenColumns} = debouncedState
    const val = {
      sortBy,
      filters,
      pageSize,
            ...
            {headerGroups.map((headerGroup) => (
              <MUITableRow {...headerGroup.getHeaderGroupProps()}>
                {headerGroup.headers.map((column) => {
                  ...
                  return (
                    <MUITableCell {...column.getHeaderProps(headerProps)}>
                      {column.canGroupBy && (
                        <TableSortLabel
                          active
            ...
            {page.map((row) => {
              ...
              return (
                <MUITableRow {...row.getRowProps()}>
                  {row.cells.map((cell) => (
                    <MUITableCell {...cell.getCellProps(cellProps)} onClick={cellClickHandler(cell)}>
                      {cell.isGrouped ? (
                        <>
                          <TableSortLabel
                            classes={{
